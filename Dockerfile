...
RUN addgroup -g 513 docker && \
    apk update && \
    apk add --no-cache bash ca-certificates curl gnupg libcap openssl git openssh make gcc musl-dev libffi-dev libssl-dev python-dev docker py-pip jq gettext && \
